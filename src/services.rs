...
pub fn scan_media(
    ...
    if let Ok(true) = config::mcat_dir_path().try_exists() {
        return Err(McatError::RepeatedInit);
    }
    fs::create_dir_all(config::lrc_dir_path())?;
    fs::create_dir_all(config::cover_dir_path())?;
